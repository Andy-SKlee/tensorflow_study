...
# CIFAR-10 데이터 다운, float32 타입으로 변경, [0, 1]로 normalize
...
class CNN(object):
    def __init__(self):
        # 첫번째 convolutional layer - 하나의 RGB 이미지를 64개의 특징들(feature)으로 맵핑(maping)
        ...
        # 두번째 convolutional layer - 64개의 특징들(feature)을 64개의 특징들(feature)로 맵핑(maping)
    ...
    def __call__(self, x, rate):
        ...
        h_conv5 = tf.nn.relu(tf.nn.conv2d(h_conv4, self.W_conv4, strides=[1, 1, 1, 1], padding='SAME') + self.b_conv5)
...
for i in range(10000):
    ...
    # 100 스텝마다 training 데이터셋에 대한 정확도와 loss cnffur
    if i % 100 == 0:
