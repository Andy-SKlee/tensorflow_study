...
class CNN(object):
  ...
  def __init__(self):
    ...
    # Output Layer
    # 1024개의 특징들(feature)을 10개의 클래스 one-hot encoding으로 표현된 숫자 0~9로 변
...
# tf.train.CheckpointManager를 이용하여 파라미터 저장
SAVER_DIR = "./model"
...
# 만약 저장된 모델과 파라미터가 있으면 불러오고(Restore)
# Restored 모델을 이용해서 텍스트 데이터에 대한 정확도를 출력하고 프로그램 종료
if latest_ckpt:
...
for step in range(10000):
    ...
    # 100 step마다 training 데이터셋에 대한 정확도를 출력하고 tf.train.CheckpointManager를 이용해서 파라미터 저장
    if step % 100 == 0:
