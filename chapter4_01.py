...
# 이미지들을 flaot32 데이터 타입으로 변환
x_train, x_test = x_train.astype('float32'), x_test.astype('float32')
# 28 x 28 형태의 이미지를 784차원으로 flattening
...
def compute_accuracy(y_pred, y):
    correct_prediction = tf.equal(tf.argmax(y_pred, 1), tf.argmax(y, 1))    # 인덱스로 맥스값 비교
    accuracy = tf.reduce_mean(tf.cast(correct_prediction, tf.float32))  # 부동소수점형에서 정수형으로 바꾼 경우 소수점 버림을 하고, Boolean형태인 경우 True이면 1, False이면 0을 출력한다
    return accuracy
